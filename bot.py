...
COMANDOS ESPECIALES:
- "gaste X en Y" -> GASTO_REGISTRADO|monto|categoria|descripcion
- "gane X" -> INGRESO_REGISTRADO|monto|descripcion
- "presupuesto X 300000" -> PRESUPUESTO_DEFINIDO|categoria|monto
- "mi correo es X" -> EMAIL_CONFIGURADO|correo
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if reply.startswith("EMAIL_CONFIGURADO|"):
        parts = reply.split("|")
        if len(parts) >= 2:
            try:
                email = parts[1].strip()
