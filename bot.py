...
# Base de datos en memoria por usuario
user_data = {}
user_histories = {}
...
def generar_grafica(user_id):
    ...
    if not por_categoria and total_ingresos == 0:
        ...
        # 1. Grafica de pastel - gastos por categoria
        ax1 = fig.add_subplot(gs[0, :])
        ax1.set_facecolor('#16213e')
        if por_categoria:
            colores = ['#e94560', '#0f3460', '#533483', '#e94560', '#06d6a0', 
                      '#ffd166', '#ef476f', '#118ab2', '#073b4c', '#26547c']
            ...
            ax1.axis('off')
            ax1.set_title('Gastos por Categoria', color='white', fontsize=12)
        ...
        colores_barras = ['#06d6a0', '#e94560']
        bars = ax2.bar(categorias, valores, color=colores_barras, width=0.5, edgecolor='none')
        for bar, val in zip(bars, valores):
            ax2.text(bar.get_x() + bar.get_width()/2., bar.get_height() + max(valores)*0.02,
                    f'{moneda}{val:,.0f}', ha='center', va='bottom', 
        ...
        ax3.text(0.5, 0.35, f'{moneda}{abs(balance):,.0f}', ha='center', va='center',
                fontsize=18, color=color_balance, fontweight='bold', transform=ax3.transAxes)
        estado = 'Positivo' if balance >= 0 else 'Negativo'
        ax3.text(0.5, 0.15, estado, ha='center', va='center',
                fontsize=11, color=color_balance, transform=ax3.transAxes)
        ax3.axis('off')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Puedo ayudarte a controlar tus finanzas de forma simple.\n\n"
        "Escribe libremente o usa el menu:",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "menu_gasto":
        await query.message.reply_text(
            ...
            "- 30000 transporte\n"
            "- /gasto 80000 entretenimiento"
        )
    elif query.data == "menu_ingreso":
        await query.message.reply_text(
            "Para registrar un ingreso escribe:\n\n"
            "Ejemplos:\n"
            "- Recibi 2000000 de salario\n"
            "- Gane 500000 freelance\n"
            "- /ingreso 1500000"
        )
        ...
        if not data["metas"]:
            await query.message.reply_text(
                "No tienes metas aun.\n\nEscribe algo como:\n"
                "- Quiero ahorrar 1000000 para vacaciones en 6 meses\n"
                "- Meta: comprar moto 3000000 en 12 meses"
            )
        else:
            texto = "Tus metas de ahorro:\n\n"
            for m in data["metas"]:
                texto += f"🎯 {m['nombre']}: ${m['objetivo']:,.0f}\n"
...
async def mostrar_resumen(message, user_id):
    ...
    texto += "━━━━━━━━━━━━━━━\n"
    texto += f"💰 Ingresos: {moneda}{total_ingresos:,.0f}\n"
    texto += f"💸 Gastos:   {moneda}{total_gastos:,.0f}\n"
    texto += f"{'📈' if balance >= 0 else '📉'} Balance:  {moneda}{balance:,.0f}\n"
    
    if por_categoria:
        texto += "\n📂 Por categoria:\n"
        for cat, monto in sorted(por_categoria.items(), key=lambda x: x[1], reverse=True):
            texto += f"  • {cat}: {moneda}{monto:,.0f}\n"
...
async def pedir_consejo_ia(user_id, total_gastos, total_ingresos, balance, por_categoria):
    contexto = f"El usuario tiene estos datos del mes: ingresos={total_ingresos}, gastos={total_gastos}, balance={balance}, gastos por categoria={por_categoria}. Da un consejo financiero corto y util (max 3 oraciones), personalizado a sus datos."
    try:
        resp = requests.post(
            "https://api.anthropic.com/v1/messages",
            headers={"x-api-key": ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json"},
            json={"model": "claude-haiku-4-5-20251001", "max_tokens": 300, "system": SYSTEM_PROMPT, "messages": [{"role": "user", "content": contexto}]},
            timeout=30
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        resp = requests.post(
            "https://api.anthropic.com/v1/messages",
            headers={"x-api-key": ANTHROPIC_API_KEY, "anthropic-version": "2023-06-01", "content-type": "application/json"},
            json={"model": "claude-haiku-4-5-20251001", "max_tokens": 1000, "system": SYSTEM_PROMPT, "messages": user_histories[user_id]},
            timeout=30
        )
