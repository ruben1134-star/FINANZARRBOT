...
SYSTEM_PROMPT = """Eres FinBot, asistente financiero personal en Telegram.

REGLAS DE FORMATO — CRITICO:
- NUNCA uses ** negritas **, tablas con |---| ni markdown de ningun tipo.
- Solo texto plano con emojis. Respuestas cortas (max 2 oraciones despues del token).
- El token SIEMPRE va en la PRIMERA linea, sin nada antes.
...
EJEMPLOS CORRECTOS:
  Usuario: "gaste 15000 en almuerzo"
  Respuesta:
  GASTO_REGISTRADO|15000|Comida|almuerzo
  Anotado! Come bien 😄

  Usuario: "gane 3000000"
  Respuesta:
  INGRESO_REGISTRADO|3000000|Ingreso
  Excelente ingreso! A administrarlo con cabeza 💪

  Usuario: "debo 80000 a Carlos por el taxi"
  Respuesta:
  DEUDA_REGISTRADA|debo|80000|Carlos|taxi
  Deuda anotada, recuerda saldarla pronto 👍

  Usuario: "me equivoque" / "borra el ultimo" / "eso estuvo mal"
  Respuesta:
  BORRAR_ULTIMO_GASTO
  Listo, lo borro!

Para preguntas generales responde en espanol, amable, sin markdown."""
...
def get_deudas(user_id):
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT * FROM deudas WHERE user_id=%s AND pagada=FALSE ORDER BY fecha DESC", (user_id,))
            return cur.fetchall()
...
def get_gastos_fijos(user_id):
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT * FROM gastos_fijos WHERE user_id=%s AND activo=TRUE", (user_id,))
            return cur.fetchall()
...
def verificar_alerta_presupuesto(user_id, categoria):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT COALESCE(SUM(monto),0) FROM gastos WHERE user_id=%s AND categoria=%s AND fecha>=%s",
...
def get_historial(user_id, limit=20):
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT role,content FROM historial WHERE user_id=%s ORDER BY created_at DESC LIMIT %s",
                        (user_id, limit))
...
def get_metas(user_id):
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT id,nombre,objetivo,actual FROM metas WHERE user_id=%s", (user_id,))
            return cur.fetchall()
...
def get_ultimos_movimientos(user_id, dias=7):
    ...
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("""SELECT fecha,categoria,descripcion,monto,'gasto' AS tipo FROM gastos WHERE user_id=%s AND fecha>=%s
                           UNION ALL
                           SELECT fecha,'Ingreso' AS categoria,descripcion,monto,'ingreso' AS tipo FROM ingresos WHERE user_id=%s AND fecha>=%s
                           ORDER BY fecha DESC LIMIT 15""", (user_id, inicio, user_id, inicio))
            return cur.fetchall()
...
def _llamar_ia(messages, max_tokens=500):
    resp = requests.post(
...
async def pedir_consejo_ia(user_id):
    ...
    ctx = (f"Finanzas: ingresos=${ti:,.0f}, gastos=${tg:,.0f}, balance=${bal:,.0f}, "
           f"categorias={pc}, racha_ahorro={racha} dias, gasto_inusual_hoy={inusual}. "
           f"Da un consejo muy corto (max 2 oraciones), motivador, en espanol, sin markdown.")
    try:
        return _llamar_ia([{"role":"user","content":ctx}], max_tokens=200)
    except Exception:
...
async def scheduler(bot):
    ...
    while True:
        try:
            ahora = colombia_now()
            hoy   = ahora.date()
            if ahora.hour == 8 and ahora.minute < 10 and enviado_manana != hoy:
                enviado_manana = hoy
                await enviar_buenos_dias(bot)
            if ahora.hour == 20 and ahora.minute < 10 and enviado_noche != hoy:
                enviado_noche = hoy
                await enviar_resumenes_noche(bot)
            if hoy.day == 1 and ahora.hour == 9 and ahora.minute < 10 and backup_mes != hoy.month:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if tl in ("/grafica","grafica","grafica"):
    ...
    if tl in ("/prediccion","prediccion","prediccion"):
        p = texto_prediccion(user_id)
