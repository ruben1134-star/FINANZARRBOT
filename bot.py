...
def registrar_usuario(user_id):
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO usuarios (user_id) VALUES (%s) ON CONFLICT DO NOTHING",
                (user_id,)
            )
        conn.commit()
...
def get_resumen(user_id):
    ...
    return total_gastos, total_ingresos, total_ingresos - total_gastos, por_categoria
...
def get_resumen_dia(user_id, fecha):
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            rows = cur.fetchall()
            por_categoria = {r["categoria"]: float(r["total"]) for r in rows}
    return total_gastos, total_ingresos, por_categoria
...
def get_resumen_semana(user_id):
    ...
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT COALESCE(SUM(monto),0) FROM gastos WHERE user_id=%s AND fecha >= %s AND fecha <= %s", (user_id, inicio_semana, hoy))
            total_gastos = float(cur.fetchone()[0])
            cur.execute("SELECT COALESCE(SUM(monto),0) FROM ingresos WHERE user_id=%s AND fecha >= %s AND fecha <= %s", (user_id, inicio_semana, hoy))
            total_ingresos = float(cur.fetchone()[0])
            cur.execute("SELECT categoria, SUM(monto) as total FROM gastos WHERE user_id=%s AND fecha >= %s AND fecha <= %s GROUP BY categoria ORDER BY total DESC", (user_id, inicio_semana, hoy))
            rows = cur.fetchall()
            por_categoria = {r["categoria"]: float(r["total"]) for r in rows}
    return total_gastos, total_ingresos, total_ingresos - total_gastos, por_categoria
...
def generar_grafica(user_id):
    ...
    if not por_categoria and total_ingresos == 0:
        ...
        cb = '#06d6a0' if balance >= 0 else '#e94560'
        ax3.text(0.5, 0.6, f'{"↑" if balance >= 0 else "↓"} Balance', ha='center', va='center', fontsize=13, color='white', fontweight='bold', transform=ax3.transAxes)
        ax3.text(0.5, 0.35, f'${abs(balance):,.0f}', ha='center', va='center', fontsize=18, color=cb, fontweight='bold', transform=ax3.transAxes)
        ax3.text(0.5, 0.15, 'Positivo' if balance >= 0 else 'Negativo', ha='center', va='center', fontsize=11, color=cb, transform=ax3.transAxes)
        ax3.axis('off')
...
# ─────────────────────────────────────────
# RECORDATORIOS Y RESUMENES AUTOMATICOS
# ─────────────────────────────────────────

async def enviar_resumen_diario(bot: Bot):
    """Envia resumen del dia a todos los usuarios a las 8pm."""
    hoy = datetime.date.today()
    dia_semana = hoy.weekday()  # 6 = domingo
    usuarios = get_todos_usuarios()

    for user_id in usuarios:
        try:
            total_gastos, total_ingresos, por_categoria = get_resumen_dia(user_id, hoy)

            # Si es domingo, enviar resumen semanal
            if dia_semana == 6:
                tg, ti, balance, por_cat_semana = get_resumen_semana(user_id)
                texto = "📅 Resumen semanal (ultimos 7 dias)\n"
                texto += "━━━━━━━━━━━━━━━\n"
                ...
                if por_cat_semana:
                    texto += "\n📂 Por categoria:\n"
                    for cat, monto in sorted(por_cat_semana.items(), key=lambda x: x[1], reverse=True):
            ...
            # Resumen del dia (todos los dias incluyendo domingo)
            if total_gastos == 0 and total_ingresos == 0:
                # No registró nada hoy — recordatorio
                await bot.send_message(
                    chat_id=user_id,
                    text=(
                        "🔔 Recordatorio de FinBot\n\n"
                        "No registraste ningun movimiento hoy.\n"
                        "¿Gastaste o ganaste algo? Cuéntame para mantener tus finanzas al dia 📊"
                    )
                )
            else:
                # Tiene registros — mostrar resumen del dia
                texto = f"📊 Tu resumen de hoy ({hoy.strftime('%d/%m/%Y')})\n"
                texto += "━━━━━━━━━━━━━━━\n"
                if total_ingresos > 0:
                ...
                if total_gastos > 0:
                    ...
                    if por_categoria:
                        for cat, monto in sorted(por_categoria.items(), key=lambda x: x[1], reverse=True):
                            texto += f"  • {cat}: ${monto:,.0f}\n"
                balance_dia = total_ingresos - total_gastos
                texto += f"\n{'📈' if balance_dia >= 0 else '📉'} Balance del dia: ${balance_dia:,.0f}"
                await bot.send_message(chat_id=user_id, text=texto)

        except Exception as e:
            print(f"Error enviando resumen a {user_id}: {e}")

async def scheduler(bot: Bot):
    """Revisa cada minuto si es hora de enviar recordatorios (8:00 PM hora Colombia GMT-5)."""
    ya_enviado_hoy = None
    while True:
        ahora = datetime.datetime.utcnow() - datetime.timedelta(hours=5)  # GMT-5
        if ahora.hour == 20 and ahora.minute == 0:
            hoy = ahora.date()
            if ya_enviado_hoy != hoy:
                ya_enviado_hoy = hoy
                print(f"Enviando resumenes diarios - {hoy}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "• Resumen diario automatico a las 8pm\n"
...
async def post_init(application):
    """Inicia el scheduler cuando arranca el bot."""
...
def main():
    ...
    print("FinBot iniciado con recordatorios automaticos!")
